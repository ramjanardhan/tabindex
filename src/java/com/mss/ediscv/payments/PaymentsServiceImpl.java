...
public class PaymentsServiceImpl implements PaymentsService {
    ...
    public ArrayList<PaymentBean> buildpaymentSQueryArchive(PaymentsAction paymentsAction) throws ServiceLocatorException {
        StringBuffer paymentSearchQuery = new StringBuffer();
        String paDateTo = paymentsAction.getPaDateTo();
        String paDateFrom = paymentsAction.getPaDateFrom();
        String paSenderId = "";
        ...
        paymentSearchQuery.append("SELECT DISTINCT(ARCHIVE_PAYMENT.FILE_ID) as FILE_ID,ARCHIVE_FILES.TRANSACTION_TYPE,TP1.ID as Sender_ID, TP1.NAME as Sender_Name, "
                + "ARCHIVE_PAYMENT.DATE as Date, ARCHIVE_PAYMENT.Check_Amount as Check_Amount,ARCHIVE_FILES.DATE_TIME_RECEIVED as DATE_TIME_RECEIVED, "
                ...
                + "TP2.NAME as ReceiverName,ARCHIVE_FILES.ACK_STATUS as ACK_STATUS,ARCHIVE_FILES.STATUS as STATUS,ARCHIVE_FILES.REPROCESSSTATUS FROM ARCHIVE_PAYMENT ");
        paymentSearchQuery.append("LEFT OUTER JOIN ARCHIVE_FILES ON (ARCHIVE_PAYMENT.FILE_ID=ARCHIVE_FILES.FILE_ID) ");
        paymentSearchQuery.append("LEFT OUTER JOIN TP TP1 ON (TP1.ID=ARCHIVE_FILES.SENDER_ID) ");
        paymentSearchQuery.append("LEFT OUTER JOIN TP TP2 ON (TP2.ID=ARCHIVE_FILES.RECEIVER_ID) ");
        paymentSearchQuery.append("WHERE 1=1 AND FLOWFLAG like 'M' ");
        if (paDateFrom != null && !"".equals(paDateFrom)) {
            //StringTokenizer st = new StringTokenizer(paDateFrom, " ");
            //String paDateFrom1 = st.nextToken();
